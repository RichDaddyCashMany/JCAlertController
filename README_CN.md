...
只有标题:
`type`应该使用`JCAlertTypeTitleOnly`，此时`标题`的上下间距加大。
```objc
...
```
只有内容：
`type`应该使用`JCAlertTypeContentOnly`，此时`内容`的上下间距加大。
```objc
...
```
标题和内容都有:
`type`应该使用`JCAlertTypeNormal`，此时
`标题`上方的间距 == `标题`和`内容`的之间的间距 == `内容`下方的间距
```objc
...
```
自定义样式：
`JCAlertStyle`是一个单例，只需设置一次即可。你可以对其中的弹窗、背景、标题、内容、按钮、分隔线的样式属性进行修改。
。
请注意JCAlertStyle的`type`要和JCAlertController的`type`是同一个枚举值。
...
自定义`contentView`:
contentView位于标题和按钮中间，可以设置`不显示`标题或按钮来实现不同的自定义`效果`。
contentView的`宽度`必须和JCAlertStyle的`alertView.width`相同，这样才能和标题、按钮的宽度保持一致。
